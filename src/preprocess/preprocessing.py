...
class Preprocessor():
    def __init__(self, pos_input_path:str, neg_input_path, test_input_path:str, 
        ...
        self.test_path = test_input_path
        self.output_test_filename = output_test_filename
    ...
    def _process_tweets(self, train_tweets, train_labels, test_tweets):
        ...
        df = pd.DataFrame({"text": processed_train_tweets, "labels": train_labels})
        ...
        test_df = pd.DataFrame({"text": processed_test_tweets})
        path = f"{self.output_dir}/{self.output_test_filename}.csv"
    ...
    def process(self):
        ...
        if self.parallelize:
            ...
            with multiprocessing.Manager() as manager:
                ...
                path = f"{self.output_dir}/{self.output_filename}.csv"
                df.to_csv(path, index=False)
                test_processed_list = manager.list()
                pool = multiprocessing.Pool(processes=self.num_parallels, initargs=(multiprocessing.RLock(),), initializer=tqdm.set_lock)
                jobs = [pool.apply_async(func=self._parallel_process_tweets, args=(splitted_test_tweets[i], i, test_processed_list, len(splitted_test_tweets), )) for i in range(len(splitted_test_tweets))]
                pool.close()
                results = []
                for job in tqdm(jobs):
                    results.append(job.get())
                test_processed_tweets = np.array(test_processed_list)
                df = pd.DataFrame({"text": test_processed_tweets})
                path = f"{self.output_dir}/{self.output_test_filename}.csv"
                df.to_csv(path, index=False)
        else:
            train_tweets = np.concatenate([pos_tweets, neg_tweets])
            ...
            self._process_tweets(train_tweets, train_labels, test_tweets)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_test_filename")
    ...
    if args.pos_input_path is None or args.test_input_path is None or args.neg_input_path is None or args.output_dir is None or args.output_filename is None or args.output_test_filename is None:
    ...
    preprocessor = Preprocessor(pos_input_path=args.pos_input_path, neg_input_path=args.neg_input_path,
                                test_input_path=args.test_input_path, output_dir=args.output_dir,
                                output_filename=args.output_filename, output_test_filename=args.output_test_filename,
