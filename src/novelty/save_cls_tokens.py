...
def save_model_predictions(config, df):
    tokens = [np.empty(0, dtype=float) for i in range(len(df))]
    print(len(tokens))
        ...
        with tqdm(total=len(train_loader), leave=False) as pbar:
            for j, batch in enumerate(train_loader):
                ...
                for k, out in enumerate(outs):
                    ...
                    _tensor = tokens[index]
                    cat_tensor = np.concatenate((_tensor, out))
                    tokens[index] = cat_tensor
                pbar.update(1)
            model.to("cpu")
            del model
            del collator
    ...
    np.save(f"{config.general.cls_save_path}/{config.model.cls_filename}", tokens)
